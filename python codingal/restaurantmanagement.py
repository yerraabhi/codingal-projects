...
class RestaurantOrderManagement:
    ...
    def setup_background(self,root):
        ...
        canvas.pack()
        original_image=tk.PhotoImage(file="restaurant.jpg")
        background_image=original_image.subsample(original_image.width()// bg_width)
        (original_image.height()// bg_height)
        canvas.create_image(0,0,anchot=tk.NW,image=background_image)
    ...
    def place_order(self):
        ...
        for item,entry in self.menu_quantities.items():
            ...
            if quantity.isdigit():
                ...
                total_cost+=cost
                if quantity>0:
                    order_summary+=f"{item}: {quantity} x {symbol}{price}={symbol}{total_cost}\n"
        if total_cost>0:
